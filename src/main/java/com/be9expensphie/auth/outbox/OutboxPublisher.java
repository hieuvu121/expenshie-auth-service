...
public class OutboxPublisher {
    ...
    public void drain() {
        ...
        for (OutboxEvent row : batch) {
            try {
                 ...
                 * would reorder two events for the same user. The row keeps
                 * publishedAt null, so the next tick starts again from here.
                 */
    ...
    private void send(OutboxEvent row) throws Exception {
        switch (row.getTopic()) {
            case "user-events" -> userKafkaTemplate
                    .send(row.getTopic(), row.getAggregateId(),
                            objectMapper.readValue(row.getPayload(), UserEvent.class))
                    .get();
            case "email-events" -> emailKafkaTemplate
                    .send(row.getTopic(), row.getAggregateId(),
                            objectMapper.readValue(row.getPayload(), EmailEvent.class))
                    .get();
